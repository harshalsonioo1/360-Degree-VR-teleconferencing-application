...
MSE = zeros(48,size(tiling,1),42-22+1); % nUser * nubmer of tiles * (42 - 22 + 1)
viewedTiles = zeros(48,size(tiling,1)); % nUser * nubmer of tiles
...
for user=1:48%%%DEBUG
    ...
    for j=transCenter(1)-314:transCenter(1)+315
        for k=transCenter(2)-419:transCenter(2)+420
            x=[j,k];
            d=sqrt((x(1)-transCenter(1))^2+(x(2)-transCenter(2))^2);
            N=840;
            v=0.43;
            e=(atan(d/(N*v))/pi)*180;
